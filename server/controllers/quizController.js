...
exports.attemptQuiz = async (req, res) => {
  try {
    ...
    if (currentUser.attemptedQuizes.includes(quizId)) {
      return res.status(400).json({ 
        success: false, 
    ...
    console.log("Before saving - User attemptedQuizes:", user.attemptedQuizes);
    console.log("Quiz ID to add:", quizId);

    if(!user.attemptedQuizes.includes(quizId)) {
      user.attemptedQuizes.push(quizId);
      await user.save();
      console.log("After saving - User attemptedQuizes:", user.attemptedQuizes);
    } else {
      console.log("Quiz already in attemptedQuizes list");
    }
